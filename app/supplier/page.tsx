      ...
      <section className="mt-8">
        ...
        <div className="overflow-x-auto rounded-xl border border-slate-200 bg-white">
          <table className="w-full text-sm">
            ...
            <tbody>
              {(materials ?? []).length === 0 && (
                <tr><td colSpan={8} className="px-4 py-6 text-center text-slate-500">Nothing yet.</td></tr>
              )}
              {materials?.map((m) => (
                <tr key={m.id} className="border-t border-slate-100">
                  <td className="px-4 py-2 text-slate-700">{m.name}</td>
                  {/* @ts-expect-error relation */}
                  <td className="px-4 py-2 text-slate-700">{m.projects?.name ?? "—"}</td>
                  <td className="px-4 py-2 text-slate-700">{Number(m.quantity)} {m.unit}</td>
                  <td className="px-4 py-2 text-slate-700">₹{Number(m.unit_cost).toLocaleString()}</td>
                  <td className="px-4 py-2 text-slate-700">₹{lineTotal(m.quantity, m.unit_cost).toLocaleString()}</td>
      ...
      <section className="mt-8">
        ...
        <div className="overflow-x-auto rounded-xl border border-slate-200 bg-white">
          <table className="w-full text-sm">
            ...
            <tbody>
              {(payments ?? []).length === 0 && (
                <tr><td colSpan={6} className="px-4 py-6 text-center text-slate-500">No bills submitted.</td></tr>
              )}
              {payments?.map((p) => (
