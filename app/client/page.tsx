    ...
    <main className="min-h-screen">
      ...
      <div className="bg-gradient-to-br from-[var(--brand)] via-[var(--brand-deep)] to-slate-900 px-4 pb-20 pt-8 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-6xl">
          ...
          <h1 className="mt-1 text-2xl font-bold text-white sm:text-3xl">{client.name}</h1>
        </div>
      </div>
      ...
      <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 -mt-14">
        ...
        {(projects ?? []).length === 0 ? (
          ...
          <>
            {/* ── Projects ── */}
            <SectionHeader title="Your Projects" count={(projects ?? []).length} className="mt-10" />
                    ...
                    <div className="px-5 py-5 sm:px-6">
                      ...
                      <div className="grid gap-4 sm:grid-cols-3">
                        ...
                        <div className="rounded-xl bg-emerald-50 px-4 py-3">
                          <div className="text-[11px] uppercase tracking-wide text-emerald-600">Paid</div>
                          <div className="text-sm font-semibold text-emerald-700">₹{received.toLocaleString()}</div>
                        </div>
                        <div className="rounded-xl bg-amber-50 px-4 py-3">
                          <div className="text-[11px] uppercase tracking-wide text-amber-600">Remaining</div>
                          <div className="text-sm font-semibold text-amber-700">₹{Math.max(remaining, 0).toLocaleString()}</div>
                        </div>
                      </div>
                      ...
                      {projectTrend.length > 0 && (
                        <div className="mt-4 rounded-xl border border-slate-100 bg-slate-50/50 p-3">
                          <div className="mb-1 text-[11px] uppercase tracking-wide text-slate-400">Payment trend</div>
                          <CashFlowTrendChart daily={projectTrend} width={400} height={70} />
                        </div>
                      )}
                      ...
                      {p.next_payment_date && (
                        <div className="mt-4 flex items-center gap-2 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3">
                          <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="#d97706" strokeWidth="2" className="shrink-0"><path strokeLinecap="round" strokeLinejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" /></svg>
                      ...
                      {(labourCounts?.size ?? 0) > 0 || projectChangeOrders.length > 0 ? (
                        <div className="mt-4 grid gap-4 sm:grid-cols-2">
                          {labourCounts && labourCounts.size > 0 && (
                            <div>
                              <div className="mb-2 text-[11px] font-semibold uppercase tracking-wide text-slate-400">Labour on site</div>
                              <div className="flex flex-wrap gap-1.5">
                                {Array.from(labourCounts.entries()).map(([category, count]) => (
                                  <span
                                    key={category}
                                    className="rounded-lg bg-brand/5 px-2.5 py-1 text-xs font-medium text-brand-700 ring-1 ring-inset ring-brand/10"
                                  >
                          ...
                          {projectChangeOrders.length > 0 && (
                            <div>
                              <div className="mb-2 text-[11px] font-semibold uppercase tracking-wide text-slate-400">Additional work</div>
            ...
            {/* ── Site Updates ── */}
            <div id="updates">
              <SectionHeader title="Site Updates" count={(updates ?? []).length} className="mt-12" />
              {(updates ?? []).length === 0 ? (
                ...
                <div className="relative space-y-0">
                  <div className="absolute left-5 top-2 bottom-2 w-px bg-slate-200 sm:left-6" />
                  {updates?.map((u, i) => (
                    <div key={u.id} className="relative flex gap-4 pb-6 sm:gap-5">
                      <div className="relative z-10 mt-1 flex h-10 w-10 shrink-0 items-center justify-center rounded-full border-2 border-white bg-brand/10 text-brand-700 shadow-sm sm:h-12 sm:w-12">
                        {u.image_url ? (
            ...
            {/* ── Payments ── */}
            <div id="payments">
              <SectionHeader title="Payment History" count={(clientPayments ?? []).length} className="mt-12" />
